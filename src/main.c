...
/*   Updated: 2019/12/05 11:48:22 by trbonnes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(int ac, char **av)
{
	...
	if (ac == 2 || (ac == 3 && !ft_strncmp(av[2], "-save", 5)))
	{
		...
		param.texture_sprite.img_data = (int *)mlx_get_data_addr(mlx_xpm_file_to_image(param.mlx_ptr, param.west_path, &param.texture_sprite.width, &param.texture_sprite.height), &param.texture_sprite.bits_per_pixel, &param.texture_sprite.size_line, &param.texture_sprite.endian);
		if (ac < 3)
